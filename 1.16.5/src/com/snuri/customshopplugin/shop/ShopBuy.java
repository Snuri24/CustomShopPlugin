...
public class ShopBuy extends Shop {
	...
	public void open(Player player) {
		...
		ServerPacketGuiOpen packet = new ServerPacketGuiOpen("shop_buy", false);
	...
	public void onMouseEvent(Player player, String id, int mouseButton, boolean isCtrlKeyDown, boolean isAltKeyDown, boolean isShiftKeyDown) {
		...
		if(id.startsWith("b0")) { // BUY
			...
			Item item = itemList.get(idx);
			int money = CustomShopPlugin.getMoney(player.getName());
			if(money < item.getPrice()) {
				player.sendMessage(ChatColor.GOLD + "돈이 부족합니다.");
				return;
			}
			
			int amount = 1;
			if(isShiftKeyDown)
				amount = money >= item.getPrice() * 64 ? 64 : money / item.getPrice();
			int a = amount;
			...
			for(int j = 0; j < 36; j ++) {
				...
				if(t == null) {
					...
					a = 0;
					break;
				} else if(t.isSimilar(item.getItemStack())) {
					if(t.getMaxStackSize() - t.getAmount() > a) {
						t.setAmount(t.getAmount() + a);
						a = 0;
			    		break;
			    	} else {
			    		a -= t.getMaxStackSize() - t.getAmount();
			    		t.setAmount(t.getMaxStackSize());
			    		if(a == 0) break;
			    	}
				}
			}
			...
			if(q > 0) {
				...
				player.sendMessage(ChatColor.GOLD + "인벤토리 공간이 부족합니다.");
			}
			return;
		}
	}
